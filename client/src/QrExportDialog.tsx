...
export function QrExportDialog({ open, partyName, guestUrl, qrDataUrl, onClose }: QrExportDialogProps) {
  ...
  async function exportPdf() {
    ...
    try {
      ...
      const blob = await createQrPdf({ partyName, guestUrl, layout, tone, cardCount, language });
      const suffix = language === "de"
        ? layout === "poster" ? "a4-plakat" : `a4-${cardCount}-karten`
        : layout === "poster" ? "a4-poster" : `a4-${cardCount}-cards`;
      const toneSuffix = language === "de" && tone === "color" ? "farbe" : tone === "color" ? "color" : "laser";
      downloadBlob(blob, `${qrExportFileBase(partyName)}-${suffix}-${toneSuffix}.pdf`);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : t("Das PDF konnte nicht erstellt werden."));
    } finally {
      setExporting(null);
    }
  }
  ...
  async function exportPng() {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : t("Die PNG-Datei konnte nicht erstellt werden."));
    } finally {
      setExporting(null);
    }
  }
  ...
  return (
    ...
    >
      <div className="qr-export-heading">
        <div><span className="section-kicker">{t("Druckstudio")}</span><h2 id="qr-export-title">{t("QR-Code exportieren")}</h2></div>
        <button className="dialog-close" type="button" onClick={onClose} aria-label={t("Exportdialog schließen")}>×</button>
      </div>
      <p className="qr-export-intro">{t("Vorlage wählen. Herunterladen. Aufhängen.")}</p>

      <div className="qr-export-layout">
        <div className={`qr-paper-preview qr-paper-preview--${layout} qr-paper-preview--${tone}`} aria-label={t("Vorschau der gewählten Druckvorlage")}>
          {layout === "poster" ? (
            <div className="qr-preview-poster">
              ...
              <b>Scan &amp; Play</b>
              <em>{t("Song wünschen. Mitvoten.")}</em>
            </div>
          ) : (
        ...
        <div className="qr-export-options">
          <fieldset>
            ...
            <label htmlFor="qr-layout-poster" className={layout === "poster" ? "choice-card choice-card--active" : "choice-card"}>
              ...
              <strong>{t("A4-Plakat")}</strong><small>{t("Groß für Eingang oder Wand")}</small>
            </label>
            <label htmlFor="qr-layout-cards" className={layout === "cards" ? "choice-card choice-card--active" : "choice-card"}>
              ...
              <strong>{t("Kartenbogen")}</strong><small>{t("Zum Verteilen und Ausschneiden")}</small>
            </label>
          </fieldset>

          {layout === "cards" && (
            <label className="qr-count-select">
              <span>{t("Anzahl pro A4-Seite")}</span>
              <select value={cardCount} onChange={(event) => setCardCount(Number(event.target.value) as QrCardCount)}>
                {QR_CARD_COUNTS.map((count) => <option key={count} value={count}>{t(count === 2 ? "{count} große Karten" : "{count} Karten", { count })}</option>)}
              </select>
            </label>
          )}

          <fieldset>
            <legend>{t("Druckart")}</legend>
            <label htmlFor="qr-tone-color" className={tone === "color" ? "choice-card choice-card--active" : "choice-card"}>
              ...
              <strong>{t("Farbdruck")}</strong><small>{t("Violett und Orange")}</small>
            </label>
            <label htmlFor="qr-tone-mono" className={tone === "mono" ? "choice-card choice-card--active" : "choice-card"}>
              ...
              <strong>{t("Schwarzweiß / Laser")}</strong><small>{t("Klarer Kontrast, wenig Toner")}</small>
            </label>
          </fieldset>
        </div>
      </div>
      ...
      <p className="qr-export-privacy">{t("Wird lokal erstellt. Dein Gast-Link bleibt im Browser.")}</p>
      <div className="qr-export-actions">
        <button className="secondary-button" type="button" onClick={() => void exportPng()} disabled={Boolean(exporting)}>{t(exporting === "png" ? "PNG wird erstellt …" : "Nur QR als PNG")}</button>
        <button className="primary-button" type="button" onClick={() => void exportPdf()} disabled={Boolean(exporting)}>{t(exporting === "pdf" ? "PDF wird erstellt …" : "A4-PDF herunterladen")}</button>
      </div>
    </dialog>
  );
}
